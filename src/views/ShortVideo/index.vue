  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch">
      ...
      <el-form-item label="动态状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择1正常  2锁定" clearable size="small">
          <el-option
    ...
    <el-table v-loading="loading" :data="publishsList" @selection-change="handleSelectionChange" class="k_line">
      ...
      <el-table-column label="动态状态" align="center" prop="status" :formatter="statusFormat" />
     <el-table-column label="提现状态" align="center" prop="status" width="180">
      ...
      <el-table-column fixed="right" label="操作" width="500" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 3" type="primary" size="small" plain class="input" v-hasPermi="['group:group:remove']" @click="examineBtn(scope.row)">审核</el-button>
          <el-button type="primary" size="small" plain class="input" v-hasPermi="['group:group:edit']" @click="deleteThis(scope.row)">删除</el-button>
          <el-button type="primary" size="small" plain class="input" v-hasPermi="['group:group:edit']"  @click="start(scope.row)">点赞列表</el-button>
    ...
    cancel() {
      ...
      submitExamineForm() {
      	...
      	if (valid) {
      		...
      		updatePublishs(this.examineInfo).then(res=>{
