    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:redEnvelopeConfig:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:redEnvelopeConfig:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:redEnvelopeConfig:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:redEnvelopeConfig:export']"
        >导出</el-button>
      </el-col>
	  <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="redEnvelopeConfigList" @selection-change="handleSelectionChange">
...
<script>
import { listRedEnvelopeConfig, getRedEnvelopeConfig, delRedEnvelopeConfig, addRedEnvelopeConfig, updateRedEnvelopeConfig, exportRedEnvelopeConfig } from "@/api/system/redEnvelopeConfig";

export default {
  name: "RedEnvelopeConfig",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRedEnvelopeConfig(this.queryParams).then(response => {
        this.redEnvelopeConfigList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getRedEnvelopeConfig(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRedEnvelopeConfig(this.form).then(response => {
            ...
            addRedEnvelopeConfig(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除所选的的数据项?', "警告", {
          ...
          return delRedEnvelopeConfig(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有红包全局配置数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportRedEnvelopeConfig(queryParams);
        }).then(response => {
