    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:avatar:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:avatar:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="avatarList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="图片" align="center">
      	<template slot-scope="scope">
      		<el-image style="width: 100px; height: 100px"  :src="imageUrl+'/file/image/'+avatarList[scope.$index].realName" fit="scale-down" @click="watch(scope.row)"></el-image>
      	</template>
      </el-table-column>
...
<script>
import { listAvatar, getAvatar, delAvatar, addAvatar, updateAvatar, exportAvatar ,	uploadUrl,getConfig} from "@/api/system/avatar";
...
export default {
  ...
  data() {
    return {
      ...
      imgs: "",
      imgsVisible: false,
      uploadPath:"/api/storage/uploadAvatar",
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAvatar(storageId).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有本地存储数据项?', "警告", {
          ...
          return exportAvatar(queryParams);
        }).then(response => {
