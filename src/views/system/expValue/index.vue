<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="金额" prop="creatTime">
    ...
    <el-table v-loading="loading" :data="valueList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="经验" align="center" prop="name" >
        <template slot-scope="scope">
          <span>{{scope.row.name + '经验'}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="创建时间" align="center" prop="creatTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.creatTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="经验" prop="name">
          <el-input-number v-model="form.name" placeholder="请输入经验值" />
        </el-form-item>
        <el-form-item label="金额" prop="money">
...
<script>
import { listValue, getValue, delValue, addValue, updateValue, exportValue } from "@/api/system/exp";
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有购买经验值价格配置数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
